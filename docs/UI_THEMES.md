...
眼镜预览。眼镜卡片与当前媒体库卡片使用更浅的白色填充和轻度背景模糊，保留
玻璃边缘高光，让背景更清晰地透过卡片。
...
重新匹配。明暗混合区域增加细描边与柔和阴影，避免文字融入图片。表单、设置
内容保留稳定的浅色面板和深色文字，眼镜卡片与媒体库卡片保持通透。
只在选图时生成一份小尺寸内存采样，后续使用这份数据；静止页面不持续采样。

Liquid 底部导航使用 6–20% 浅色填充与 8 px 背景模糊，中间触控入口也降低填充，
保留玻璃细边和当前页短横线。自定义壁纸下，三个导航项沿用自动/浅色/深色文字
配色；自动模式也考虑滚到导航栏下方的浅色内容卡片。simpleUI 继续使用实色导航。
