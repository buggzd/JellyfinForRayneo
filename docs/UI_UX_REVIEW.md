...
## 首页与剧集详情 Hero 封面尺寸对齐（2026-09-07）

首页与详情共用封面宽高变量，以首页原有的 2:3 比例、靠右和垂直居中规则为基准。液态玻璃保留 132vh，simpleUI 保留 115vh；详情页移除按屏幕宽度拉宽、额外缩放和独立高度上限，图片使用相同裁切位置。详情外层在视口高度内裁切，保留原有文字、按钮、列表和主题渐变。

合成封面的浏览器验证覆盖两种主题的 1024×640、1440×810、1920×1080、1440×1000：首页与详情的实际图片宽高、坐标一致，没有横向溢出，操作按钮和唯一遥控焦点正常。当时 simpleUI 进入剧集列表后仍卸载 Hero 封面，回到 Hero 后恢复（该行为已由下节修正）；液态玻璃保留列表的全局封面背景。本次真机回归未执行。

TypeScript、20 项前端测试、80 项 JVM 测试、两端生产构建、Debug lint、APK 组装及 `verify-android.sh` 均通过。

## simpleUI 详情封面焦点修正（2026-09-07）

移除详情 Hero 封面随焦点卸载的条件。焦点进入下方按钮、详情标签、季选择、剧集或相关推荐时保留同一张封面，随页面滚动自然离开视口。沿用与首页对齐的尺寸和现有主题渐变；首页内容行的背景卸载与液态玻璃的封面预览行为保持原样。

旧生产资源已复现相关推荐标签聚焦后封面消失。更新后的合成媒体回归覆盖两种主题的 1024×640、1440×810、1920×1080、1440×1000，验证按钮与标签点击、上下左右导航、剧集及相关推荐聚焦、下方控件处切换主题、页面滚动和返回首页。封面 DOM 与图片地址保持不变，首页与详情尺寸仍一致；遥控焦点唯一，无页面错误、横向溢出或播放请求。

TypeScript、20 项前端测试、两端生产构建、Debug lint、APK 组装及 `verify-android.sh` 通过；JVM 测试任务复用 80 项既有通过结果。本次未执行 Android / RayNeo 真机回归。

## 眼镜设置与字幕大小（2026-09-07）

眼镜侧栏新增「设置」，集中提供液态玻璃 / simpleUI 和小、标准、大、特大四档字幕大小。主题卡展示两种外观；字号使用与实际字幕相同的响应式尺寸，并提供中英文预览。设置页支持方向导航与单击选择，返回后焦点恢复到设置入口。短视口下手势提示随页面底部排布，避让设置说明。

字号仅在设置页调整，播放控件和字幕轨道菜单沿用原有结构。播放器读取保存的大小，标准档保留现有字号；已固定在视频画面里的字幕不受影响。手机设置同步提供四档选项，恢复默认偏好时回到标准。

两端共用原生保存值，眼镜桥只接受精确的主题和字号枚举。偏好独立于账号，重建与退出登录后保留；收到原生回执后更新选中状态，不重载媒体库、视频或播放计划。独立浏览器预览仅保存专用偏好键。

合成媒体浏览器回归覆盖两种主题的 1024×640、1440×810、1920×1080，手机 320、390、430 像素宽度，设置进出与焦点恢复、四档字号、两端互改、暂停时主题 / 字号同步、直放与 HLS 的字号应用和减少动态效果。确认播放页没有字号选项，保留单个视频、原播放地址和播放开始上报次数，未出现页面错误或横向溢出。

22 项眼镜前端测试、12 项开发联调测试、83 项 JVM 测试、TypeScript、两端生产构建、Debug lint、APK 组装及 `verify-android.sh` 通过。本次检查时没有在线 ADB 设备，Android / RayNeo 真机及 2D / SBS 光学回归仍待执行。
...
### 背景裁切与通透卡片（2026-09-07）

- 背景编辑新增 0–100% 透明度、五种裁切比例、1–3 倍缩放和双轴定位，支持拖动、
  方向键与滑杆；界面预览按手机屏幕比例显示最终铺满效果。取消保留原设置，
  应用后保存；导入图片不因裁切被破坏，换图重新居中并保留透明度。
- 原生校验当前图片版本及所有参数，只回传手机状态；浏览器将图片和参数一起
  保存到 IndexedDB。Android 返回键优先退出编辑器，关闭时恢复入口按钮焦点。
- Liquid 眼镜卡片与当前媒体库卡片降低白色填充，将背景模糊从 27 px 降至 4 px，
  保留边缘高光与官方眼镜预览。simpleUI 保持原有实色样式。

浏览器使用演示账号、合成网格图与仓库自带图片，覆盖 320×568、360×640、393×852、
430×932：五种比例的实际预览几何、缩放与定位边界、透明度 0/100%、应用后裁切
与预览一致、取消/重置、键盘定位及 Escape 焦点恢复、刷新持久化、换图与清除。
小屏无横向溢出，编辑区可滚动且保存按钮固定可见。simpleUI 排除自定义图片，
切回 Liquid 恢复；纯黑触控板无自定义背景节点，主页面底色仍为 RGB 0,0,0。
浏览器无警告或错误。

6 项手机裁切测试覆盖拖动换算、横竖图与极窄图边界、参数校验和换图重置；
25 项眼镜前端测试、13 项联调测试、94 项 JVM 测试通过。TypeScript、双端生产构建、
Debug lint（零问题）、Debug/Release APK 组装和源码/APK 边界检查通过。
本轮未连接 Android/RayNeo 设备；新增编辑器的真机拖动、原生回执、冷启动及 renderer
恢复，还有完整设备矩阵仍待验收，上节真机记录不代表本轮版本已经完成这些检查。

### 背景文字自适应（2026-09-07）

背景编辑器新增自动、浅色、深色文字。自动模式按文字实际位置、最终裁切与
透明度比较对比度，透明卡片计入玻璃填充；分组独立选择明暗，接近切换边界时
保留上次颜色。混合明暗区域增加细描边与阴影。设置内容与导航保留浅色面板，
设备页两张通透卡片的填充和模糊不增加。旧裁切记录保留全部参数并补上自动配色。

浏览器使用用户提供的深色图片与演示账号验证，图片及截图不进入仓库。覆盖
0/100% 透明度、同图不同区域的颜色、手动覆盖、保存与重载、320–430 像素宽度的
编辑器、取消编辑与滚动后的文字颜色，以及 simpleUI/触控板的自适应处理和样式停用。
浏览器无警告或错误。13 项手机测试、25 项眼镜测试、13 项联调测试、95 项 JVM
测试通过；双端生产构建、Debug lint 与 Debug/Release APK 检查通过。
每张图只生成一次小尺寸采样，静止时
没有采样循环；本轮没有连接真机，Android 私有资源的采样、原生保存回执与完整
设备回归矩阵仍待验证。

### Liquid 导航栏透明度（2026-09-07）

底部导航移除自定义壁纸下的 92% 浅色填充，改用 6–20% 轻薄渐变，模糊从
30 px 降至 8 px；中间触控入口同步降低填充。三个导航项沿用文字配色偏好，
自动模式同时判断壁纸和滚动到导航栏下方的浅色内容面板。当前页用短横线标记，
保持原有触控尺寸与导航间距。

浏览器覆盖默认背景、用户提供的暗色壁纸、设置页滚动、三个入口、simpleUI
往返及 320/360/393/430 像素宽度。导航栏没有横向溢出，自动文字在浅色面板上
变深、露出暗色壁纸后变浅，浏览器无警告或错误。14 项手机测试、TypeScript、
两端生产构建、JVM/lint、Debug/Release APK 组装与边界检查通过；本轮未做真机回归。
