...
- 「设置 → 媒体服务器 → 管理」和账号卡片上的「管理」都会打开服务器与账号列表，进入页面不会退出当前连接。
...
显示模式选择、当前启用状态和切换结果统一放在「设置 → 显示」。设备页保留眼镜连接状态、触控板入口和当前媒体会话。选中虚拟银幕只表示保存了偏好；只有实际状态为已启用时，靠近程度和大小才会作用于眼镜。失败后可点击「重新启用」发起一次切换。
