...
export default function RemoteTutorial({ onExit, onComplete }: { onExit: (outcome: TutorialOutcome) => void; onComplete?: () => void }) {
        ...
        {state.phase === 'welcome' ? (
          <main className="tutorial-welcome">
            <div className="tutorial-welcome__copy">
              <span className="tutorial-eyebrow"><i /> 你的第一堂遥控课</span>
              <h1>目光留在这里。<br /><em>操作交给拇指。</em></h1>
              <p>拿好手机，打开触控板。<br />跟着眼镜里的提示，用六个小动作学会遥控。</p>
              <div className="tutorial-welcome__facts"><span><Smartphone size={19} /> 单手就能操作</span><span><Sparkles size={19} /> 大约 1 分钟</span></div>
              {choices('单击，开始练习', '稍后再学')}
              <small className="tutorial-choice-hint">左右滑动选择 · 单击确认 · 双击跳过</small>
            </div>
          ...
          <main className="tutorial-complete">
            ...
            <span className="tutorial-eyebrow">六个动作，全部点亮</span>
            <h1>你的私人影院，<em>现在听你的。</em></h1>
            <p>去挑一部喜欢的。遥控已经在你手里。</p>
            <div className="tutorial-recap">
              <div><Move size={26} /><strong>滑动</strong><span>移动焦点</span></div>
              <div><Check size={26} /><strong>单击</strong><span>确认选择</span></div>
              <div><RotateCcw size={26} /><strong>双击</strong><span>返回上一级</span></div>
            </div>
            {choices('开始浏览', '再练一次')}
            <small className="tutorial-choice-hint">随时可从眼镜端侧栏「遥控教学」重新练习</small>
          </main>
        ) : (
          <main className="tutorial-lesson">
            <section className="tutorial-instruction" aria-labelledby="tutorial-title">
              <span className="tutorial-eyebrow"><span>{String(state.step + 1).padStart(2, '0')} / 06</span>{state.step < 4 ? '移动焦点' : '选择与返回'}</span>
              <h1 id="tutorial-title">{lesson.title}</h1>
              <p>{lesson.description}</p>
              <div className="tutorial-demonstration">
                <GestureArt gesture={lesson.gesture} label={`拇指${tutorialInputLabels[lesson.command]}手机触控板的动画示范`} />
              ...
              <div className={`tutorial-feedback ${state.feedback}`} role="status" aria-live="polite">
                ...
                <span>{success ? lesson.success : state.feedback === 'retry' && state.lastInput
                  ? `收到${tutorialInputLabels[state.lastInput]}，这次试试${tutorialInputLabels[lesson.command]}。`
                  : lesson.hint}</span>
              </div>
            </section>

            <section className="tutorial-practice" aria-label="遥控练习空间">
              <div className="tutorial-practice__header"><span><i /> 练习空间</span><small>{state.step < 4 ? '让光点抵达目标' : state.step === 4 ? '选中的卡片，可以打开' : '打开的页面，可以返回'}</small></div>
              {state.step < 4 ? (
                <div className="tutorial-grid" role="group" aria-label="方向练习区">
                  {Array.from({ length: 9 }, (_, index) => {
                    ...
                    return (
                      <div
                        ...
                        aria-label={focused ? '当前光点' : target ? '发光目标' : undefined}
                      >
                        {focused ? <><span className="tutorial-light"><Check size={27} /></span><small>{success ? '已抵达' : '你在这里'}</small></>
                          : target ? <><CommandIcon size={30} /><small>移到这里</small></> : <span className="tutorial-cell__point" />}
                      </div>
                    )
                  })}
                </div>
                ...
                <div className="tutorial-card-scene">
                  <button type="button" data-tutorial-focus="practice" data-focusable="true" className="tutorial-movie-card" onClick={() => dispatch({ type: 'command', command: 'enter' })}>
                    ...
                    <div className="tutorial-movie-card__copy"><span><small>你的第一张练习卡片</small><strong>蓝色之外</strong></span><ChevronRight size={25} /></div>
                  </button>
                  <span className="tutorial-stage-note">{success ? '已经回到卡片列表' : '白色边框，就是你当前的选择'}</span>
                </div>
              ) : (
                <div className="tutorial-detail-scene">
                  <div className="tutorial-detail">
                    ...
                    <span className="tutorial-detail__badge"><Check size={17} /> 卡片已打开</span>
                    <h2>蓝色之外</h2><p>每一段旅程，都从一次轻触开始。</p>
                    <div tabIndex={0} data-tutorial-focus="practice" data-focusable="true" className="tutorial-return-target"><RotateCcw size={23} /><span>双击触控板，回到卡片</span></div>
                  </div>
                  <span className="tutorial-stage-note">这是练习用的详情页，放心试一试</span>
                </div>
              )}
              <div className={`tutorial-stage-status ${success ? 'is-done' : ''}`} aria-hidden="true">{success ? <Check size={18} /> : <span className="tutorial-status-ring" />}{success ? '完成！' : '等待你的操作'}</div>
            </section>
          </main>
        )}

        <footer className="tutorial-footer">
          <div className="tutorial-progress" aria-label={`已完成 ${completedSteps} 项，共 6 项`}>
          ...
          <span className="tutorial-footer__hint">{state.phase === 'practice' ? state.step === 5 ? '快速双击 · 返回练习卡片' : '双击 · 暂停或退出教学' : '滑动选择 · 单击确认'}</span>
        </footer>
      </div>
      ...
      {state.exitOpen && (
        <div className="tutorial-exit-layer">
          <section className="tutorial-exit" role="dialog" aria-modal="true" aria-labelledby="tutorial-exit-title">
            ...
            <h2 id="tutorial-exit-title">要先去看看吗？</h2>
            <p>已完成 {completedSteps} / 6 个动作。<br />随时可以从侧栏「遥控教学」重新开始。</p>
            {choices('继续练习', '退出教学')}
            <small className="tutorial-choice-hint">左右滑动选择 · 单击确认 · 双击继续练习</small>
          </section>
        </div>
      )}
    </div>
